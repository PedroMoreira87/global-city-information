...
export const handler = async (event) => {
	...
	try {
		...
		if (event.triggerSource === "PreSignUp_SignUp") {
			...
			console.log(
				"No external provider account found. Proceeding with sign-up.",
			);
			...
			const providerName =
				rawProviderName.charAt(0).toUpperCase() +
				rawProviderName.slice(1).toLowerCase();
			if (existingUsers.Users && existingUsers.Users.length > 0) {
				const existingUser = existingUsers.Users[0];
				console.log(
					`Found existing user with username: ${existingUser.Username}`,
				);
