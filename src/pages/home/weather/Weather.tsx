...
import {
	...
	updateUser,
	weatherData,
} from "../../../apis/aws-lambda-functions.ts";
import type { City } from "../../../interfaces/city.interface.ts";
import type {
	IWeather,
	Place,
	User,
} from "../../../interfaces/user.interface.ts";
import type { WeatherAPI } from "../../../interfaces/weather-api.interface.ts";
...
const Weather: React.FC<WeatherProps> = ({ onUserDataUpdate }) => {
	...
	const [weatherAPI, setWeatherAPI] = useState<WeatherAPI | undefined>(
		undefined,
	);
	...
	const handleSelectChange = async (event: SelectChangeEvent) => {
		...
		try {
			...
			toast.success(
				`Weather data loaded successfully for ${selectedCity.name}!`,
			);
		} catch (error: unknown) {
			if (error instanceof Error) {
				console.log(error);
					...
					<Box sx={{ width: 331.75 }}>
						<FormControl fullWidth>
							...
							<Select
								...
								id="places-found"
								value={
									placesFoundSelect ? JSON.stringify(placesFoundSelect) : ""
								}
								label="Places Found"
